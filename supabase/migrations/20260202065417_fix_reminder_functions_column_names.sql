/*
  # Fix Reminder Functions Column Names

  1. Changes
    - Fix column references in reminder functions
    - Use `reporter_id` instead of `created_by`
    - Use `assigned_to_id` instead of `assigned_to`
    
  2. Purpose
    - Ensure functions work with actual database schema
*/

-- Drop and recreate the pending assignment reminders function with correct column names
...
DECLARE
  ...
  v_errors TEXT[] := '{}';
BEGIN
  -- Find reports in 'reported' status without assigned_to_id or proposed_closure_date
  FOR v_report IN
    SELECT 
      r.id,
      r.company_id,
...
-- Drop and recreate the deadline reminders function with correct column names
