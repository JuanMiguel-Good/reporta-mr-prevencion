/*
  # Add Automated Reminder Functions

  1. New Functions
    - `send_pending_assignment_reminders()` - Sends reminders for reports without assigned_to/proposed_closure_date
      - Triggers on day 2, then every 2 days until assigned
      - Notifies SST Manager
    
    - `send_deadline_reminders()` - Sends reminders for approaching closure deadlines
      - 3 days before, 1 day before, on the day
      - After deadline: every 2 days until evidence is uploaded
      - Notifies both SST Manager and Responsible person

  2. Purpose
    - Ensure reports are assigned promptly
    - Ensure evidence is submitted before/on deadline
    - Automate follow-up notifications

  3. Security
    - Functions use SECURITY DEFINER to bypass RLS
    - Only accessible via scheduled jobs or service role
*/

-- Function to send reminders for reports without assignment
...
DECLARE
  ...
  v_errors TEXT[] := '{}';
BEGIN
  -- Find reports in 'reported' status without assigned_to or proposed_closure_date
  FOR v_report IN
    SELECT 
      r.id,
      r.company_id,
    ...
    WHERE r.status = 'reported'
      AND (r.assigned_to IS NULL OR r.proposed_closure_date IS NULL)
      AND c.active = true
  LOOP
    -- Calculate days since creation
    v_days_since_creation := EXTRACT(DAY FROM (CURRENT_TIMESTAMP - v_report.created_at));
              ...
              <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                ...
                <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
                  ...
                  <tr>
                    ...
                    <td style="padding: 8px 0;">' || COALESCE(v_report.creator_name, 'Desconocido') || '</td>
                  </tr>
                  <tr>
...
-- Function to send reminders for approaching deadlines
...
BEGIN
  -- Find assigned reports with proposed_closure_date
  FOR v_report IN
    SELECT 
      r.id,
      r.company_id,
    ...
    JOIN companies c ON r.company_id = c.id
    LEFT JOIN categories cat ON r.category_id = cat.id
    LEFT JOIN users responsible ON r.assigned_to = responsible.id
  ...
  LOOP
    v_should_send := false;
    v_days_until_deadline := EXTRACT(DAY FROM (v_report.proposed_closure_date::date - CURRENT_DATE));
    ...
    ELSIF v_days_until_deadline < 0 THEN
      -- After deadline, send every 2 days
