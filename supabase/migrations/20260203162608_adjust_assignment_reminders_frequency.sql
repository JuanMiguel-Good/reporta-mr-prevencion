/*
  # Adjust Assignment Reminders Frequency

  1. Purpose
    - Change frequency of pending assignment reminders
    - From: Day 2, then every 2 days (2, 4, 6, 8...)
    - To: Every day after 24 hours (1, 2, 3, 4...)

  2. Changes
    - Update `send_pending_assignment_reminders()` function
    - Remove the modulo 2 check
    - Start reminders from day 1 instead of day 2

  3. Result
    - SST Managers will receive daily reminders for unassigned reports
    - Reminders start 24 hours after report creation
    - More frequent follow-up ensures faster assignment
*/

-- Update function to send reminders daily instead of every 2 days
...
BEGIN
  ...
  LOOP
    ...
    -- Send reminder every day starting from day 1 (24 hours after creation)
    IF v_days_since_creation >= 1 THEN
      ...
      LOOP
        ...
        v_subject := '[' || v_report.company_name || '] Recordatorio: Reporte Sin Asignar - ' || v_days_since_creation || ' días';
...
<h2 style="margin: 0 0 8px 0; color: #92400e;">Reporte Pendiente de Asignación</h2>
...
<td style="padding: 8px 0;">' || 
TO_CHAR(v_report.created_at, 'DD/MM/YYYY HH24:MI') || 
'</td>
</tr>
</table>
